...
export default function BestTime() {
    ...
    const analyze = async (e) => {
        ...
        setLoading(true);
        try {
            const res = await api.post('/ai/best-time', { niche: topic });
            setResult(res.data);
        } catch (err) {
        ...
        return (
            <Layout>
                <div className="max-w-2xl mx-auto mt-20 text-center border border-gray-800 bg-gray-900 p-12 rounded-3xl shadow-2xl relative overflow-hidden">
                    ...
                    <p className="text-gray-400 mb-8 max-w-md mx-auto">To find the best time to post based on your real audience activity, you must first connect your YouTube handle.</p>
                    <Link 
                        to="/dashboard"
    ...
    return (
        <Layout>
            <div className="mb-10">
                <h2 className="text-3xl font-bold mb-2">Best Post Time</h2>
                <p className="text-gray-400 font-medium">Find the exact optimal upload window to maximize your initial reach.</p>
            </div>
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-8 shadow-xl mb-8">
                <form onSubmit={analyze} className="flex flex-col md:flex-row gap-4 max-w-2xl">
                    <input 
                        type="text" placeholder="Channel Category (e.g. Cooking, Tech, Gaming)"
                    ...
                    <button type="submit" disabled={loading} className="bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-bold py-3 px-8 rounded-xl hover:shadow-lg hover:shadow-blue-500/25 transition-all transform active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2 whitespace-nowrap">
                        <Clock size={18} /> {loading ? 'Calculating...' : 'Find Best Time'}
                    </button>
                </form>
            </div>
